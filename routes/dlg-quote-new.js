var selectedQuoteIdAttachmentId = null;

function doDlgQuoteNew(quoteid)
{
  var isnew = _.isUndefined(quoteid) || _.isNull(quoteid);
  ...
  {
    doServerDataMessage('newordernote', {quoteid: quoteid}, {type: 'refresh'});
  }

  function doEditorClear()
  {
    $('#divNewQuoteNotesG').datagrid('clearSelections');
  }
  ...
  function doEditorSearch()
  {
    ...
    (
      function(text)
      {
        doServerDataMessage('searchordernote', {quoteid: quoteid, words: text}, {type: 'refresh'});
      },
      function()
      {
        doServerDataMessage('listordernotes', {quoteid: quoteid}, {type: 'refresh'});
      }
    );
  }
  ...
  {
    if (quoteid == args.data.quoteid)
      doServerDataMessage('listordernotes', {quoteid: quoteid}, {ordernoteid: args.data.ordernoteid, type: 'refresh'});
  }
  ...
  {
    if (quoteid == args.data.quoteid)
      doServerDataMessage('listorderattachments', {quoteid: quoteid}, {type: 'refresh'});
  }
  ...
  function doReset()
  {
    ...
    else
    {
      if (!_.isEmpty(order))
      {
        ...
        doServerDataMessage('listorderdetails', {quoteid: quoteid, version: order.activeversion}, {type: 'refresh'});
      }
    }
    ...
    (
      ...
      function(productid, productname, qty, price, isrepeat)
      {
        ...
        else
        {
          ...
          doServerDataMessage('neworderdetail', {quoteid: quoteid, version: v, productid: productid, qty: qty, price: price, discount: null, expressfee: null}, {type: 'refresh'});
        }
      }
    );
  }
  ...
  function doNewVersion()
  {
    ...
    (
      ...
      {
        if (result)
          doServerDataMessage('newversionorder', {quoteid: quoteid, version: order.activeversion}, {type: 'refresh'});
      }
    );
  }
  ...
  {
    if (!_.isNull(quoteid))
      doServerDataMessage('loadorder', {quoteid: quoteid}, {type: 'refresh'});
  }
  ...
  function doSavedOrderDetails()
  {
    ...
    doServerDataMessage('listorderdetails', {quoteid: quoteid, version: v}, {type: 'refresh'});
  }
  ...
  function doStatusNew()
  {
    ...
    if (_.isBlank(status))
      ...
      doServerDataMessage('neworderstatus', {quoteid: quoteid, status: status, connote: connote, carrier: carrier, comment: comment, batchno: batchno}, {type: 'refresh'});
  }
  ...
  {
    doServerDataMessage('listorderstatuses', {quoteid: quoteid}, {type: 'refresh', orderstatusid: args.data.orderstatusid});
  }
  ...
  (
    {
      ...
      onOpen: function()
      {
        selectedQuoteIdAttachmentId = quoteid;
        ...
        (
          {
            ...
            onSelect: function(record)
            {
              doServerDataMessage('listorderdetails', {quoteid: quoteid, version: record.name}, {type: 'refresh'});
            }
          }
        );
        ...
        */
            ...
            onChange: function(newValue, oldValue)
            {
              if (!_.isBlank(newValue))
              {
                ...
                if (newValue != oldValue)
                  doServerDataMessage('checkorderpo', {quoteid: quoteid, pono: newValue}, {type: 'refresh'});
              }
            }
          }
        );
        ...
        if (!_.isNull(quoteid))
        {
          doServerDataMessage('loadorder', {quoteid: quoteid}, {type: 'refresh'});
          doServerDataMessage('listorderattachments', {quoteid: quoteid}, {type: 'refresh'});
          doServerDataMessage('listorderstatuses', {quoteid: quoteid}, {type: 'refresh'});
          doServerDataMessage('listordernotes', {quoteid: quoteid}, {type: 'refresh'});
        }
        else
          doReset();
